...
<div class="max-w-md mx-auto my-12 px-6 py-8 bg-[var(--color-surface)] border border-[var(--color-border)] rounded-[20px] shadow-sm">
    <div class="text-center mb-6">
        <h1 class="text-2xl font-black text-[var(--color-ink-900)] tracking-tight">Welcome Back</h1>
        <p class="text-xs text-[var(--color-ink-600)] mt-1">Sign in to manage your tokens, custom URLs, and access premium tools.</p>
    </div>
    ...
    <form action="/login" method="POST" class="space-y-4">
        ...
        <div>
            <div class="flex justify-between mb-1">
                <label class="block text-xs font-bold uppercase text-[var(--color-ink-600)]">Password</label>
                <a href="/forgot-password" class="text-xs font-bold text-[var(--color-ember-500)] hover:underline">Forgot password?</a>
            </div>
            <input type="password" name="password" required placeholder="••••••••" class="input-custom w-full h-11 px-4 text-sm">
        </div>
        ...
        <button type="submit" class="btn-ember w-full h-11 text-sm font-bold rounded-xl shadow-sm">Sign In</button>
    </form>

    <div class="text-center mt-6 pt-4 border-t border-[var(--color-border)]">
        <p class="text-xs text-[var(--color-ink-600)]">Don't have an account? <a href="/register" class="font-bold text-[var(--color-ember-500)] hover:underline">Register free</a></p>
    </div>
</div>
